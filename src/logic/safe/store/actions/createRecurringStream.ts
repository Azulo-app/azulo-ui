...
export interface CreateRecurringStreamArgs {
...
type CreateRecurringStreamAction = ThunkAction<Promise<void | string>, AppReduxState, DispatchReturn, AnyAction>
type ConfirmEventHandler = (safeTxHash: string) => void
type ErrorEventHandler = () => void
export const METAMASK_REJECT_CONFIRM_TX_ERROR_CODE = 4001

export const createRecurringStream = (
  {
    safeAddress,
    to,
    valueInWei,
    ...
    ethParameters,
  }: CreateRecurringStreamArgs,
  onUserConfirm?: ConfirmEventHandler,
  onError?: ErrorEventHandler,
): CreateRecurringStreamAction => async (dispatch: Dispatch, getState: () => AppReduxState): Promise<DispatchReturn> => {
  ...
  const newFlow = async (input) => {
    ...
    owner.flow({
      ...
      onTransaction: () => {
        ...
        console.log('input.ownerAddress', input.ownerAddress)
        console.log('input.recipientAddress', input.recipientAddress)
        console.log('input.tokenAddress', input.tokenAddress)
      },
    })
  }
