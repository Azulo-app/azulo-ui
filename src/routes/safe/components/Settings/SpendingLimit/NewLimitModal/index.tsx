...
type Step = typeof START | typeof REVIEW
...
const useNewLimitModal = (initialStep: Step): NewLimitModalHook => {
  ...
  // define actions
  const create = useCallback<ActionCallback>((newState) => dispatch({ type: START, newState, tokens }), [tokens])
...
export const NewLimitModal = ({ close, open }: SpendingLimitModalProps): ReactElement => {
  // state and dispatch
  const [{ step, txToken, values }, { create, review }] = useNewLimitModal(START)
  ...
  return (
    <Modal
      ...
      description="set rules for specific beneficiaries to access funds from this Trust without having to collect all signatures"
    >
      {step === START && <Create initialValues={values} onCancel={close} onReview={handleReview} />}
